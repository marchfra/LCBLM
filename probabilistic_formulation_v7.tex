...
\]
In practice, $1/\tau$ is a single learnable scalar (initialised so that $\tau = 0.1$, clamped to $[1, 100]$ at inference{\color{red}\footnote{\color{red}isn't it always clamped?}}), mirroring the CLIP logit-scale parameterisation; the same scale is applied uniformly to all three similarity metrics defined below.
...
\item \textbf{Mean reductions over $K$ and the embedding dimension $d$.} The factors $1/(BD)${\color{red}\footnote{\color{red}$D$ is input data dimension, right?}}, $1/(BKd)$, $1/(BK)$, $1/K$ in the loss below make each term's magnitude independent of the model dimensions, so the same $\gamma, \beta, \lambda_{\text{ent}}$ values transfer across architectures of different sizes without rescaling.
...
{\color{red}This term was later dropped since we observed empirically that its contribution to the loss was negligible, and we expect that randomly-initialized subspaces of a big vector space be orthogonal, so this was deemd redundant.}
...
The prior ratio is just the logit of the Bernoulli activation probability $\pi$. For the likelihood ratio we substitute the Gated Gaussian densities $p(z_k \mid c_k = 1) = \mathcal{N}(z_k; h_k, I)$ and $p(z_k \mid c_k = 0) = \mathcal{N}(z_k; 0, I)${\color{red}\footnote{\color{red}shouldn't it be $\sigma_0^2 I$?}}. The two distributions share the same isotropic covariance, so the $(2\pi)^{-d/2}$ normalising constants cancel inside the log-ratio:
\[
