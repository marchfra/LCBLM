# Copyright 2022 The HuggingFace Datasets Authors and the current dataset script
# contributor.
#
...
import numpy as np
import torch
from evaluate import MetricInfo, logging
...
_KWARGS_DESCRIPTION = """
Args:
    model (PreTrainedModel): model used for calculating Perplexity
    ...
    add_start_token (bool): whether to add the start token to the texts,
        so the perplexity can include the probability of the first word. Defaults to
        True.
...
Examples:
    ...
    Example 2:
        ...
        >>> perplexity = evaluate.load("perplexity", module_type="metric")
        >>> input_texts = load_dataset(
        ...     "wikitext",
        ...     "wikitext-2-raw-v1",
        ...     split="test"
        ... )["text"][:10] # doctest: +SKIP
...
class Perplexity(evaluate.Metric):
    ...
    def _compute(  # noqa: C901, PLR0912, PLR0913
        self,
        ...
        *,
        ...
        if batch_size > 1 and tokenizer.pad_token is None:
            ...
            if len(existing_special_tokens) == 0:
                msg = (
                    "If batch_size > 1, model must have at least one special token to "
                    "use for padding. Please use a different model or set batch_size=1."
                )
                raise ValueError(msg)
        ...
        if add_start_token:
            if not torch.all(torch.ge(attn_masks.sum(1), 1)):
                msg = "Each input text must be at least one token long."
                raise ValueError(msg)
        elif not torch.all(
