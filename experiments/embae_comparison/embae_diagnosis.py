...
def main() -> None:  # noqa: PLR0915
    ...
    print(f"Results : {len(results)} run(s)")
    ...
    X_train, X_test, *_ = load_data(ds_cfg.n_samples)
    ...
    inner_product_alignment_distribution(active_mask, alignments, out_dir)
    prototype_vs_avg_embedding(
        embae,
        active_mask,
        device,
        scaler,
    ...
    within_concept_embedding_variance(active_mask, embeddings, out_dir)
    prototype_embedding_distance(embae, active_mask, embeddings, out_dir)
...
def inner_product_alignment_distribution(
    active_mask: Tensor,
    alignments: Tensor,
    out_dir: Path,
) -> None:
    ...
    total_act: np.ndarray = active_mask.float().sum(dim=0).numpy()  # (n_concepts,)
    sorted_concept_idxs = np.argsort(total_act)[::-1][:MAX_CONCEPTS_SHOW]
    n_concepts = len(total_act)
...
def prototype_vs_avg_embedding(  # noqa: PLR0913
    embae: EmbeddingAE,
    active_mask: Tensor,
    device: torch.device,
    scaler,
...
def within_concept_embedding_variance(active_mask, embeddings, out_dir) -> None:
    # ── Diagnostic 3 — Within-concept encoder embedding variance ───────────────────
    print("\n=== Diagnostic 3: Within-concept encoder embedding variance ===")

    n_concepts = int(active_mask.size(1))
    ...
    ax.bar(x, within_concept_var[order], color="coral", alpha=0.8)
    ax.set_xticks(x)
    ax.set_xticklabels(
...
def prototype_embedding_distance(
    embae: EmbeddingAE,
    active_mask,
    embeddings,
    out_dir: Path,
